...
void LSM303::setAccelSensitivity(int value) {
  
  //
  // DLH: The following table denotes the allowable values that both set block update mode and accel sensitivity.
  // The table also indicates the measured nominal readings of zAxis at approx 1g (flat, non-moving).
  //
  // 0x80 = 2g/bdu z/1g = 1027-1044
  // 0x90 = 4g/bdu z/1g = 511-525
  // 0xB0 = 8g/bdu z/1g = 251-268
  //

  if((_device == LSM303DLH_DEVICE) && ((value == 0x80) || (value != 0x90) || (value != 0xB0) )) {
    writeAccReg(LSM303_CTRL_REG4_A, value);
  }
...
void LSM303::enableDefault(void)
{
  ...
  // only works on LSM303DLH_DEVICE
  setAccelSensitivity(ACCEL_2G);
